...
- **Task 2**: User must configure Railway service in Railway UI (DNS setup, service creation, domain configuration, environment variables, SSL certificate verification)
- **Task 5, 6, 9, 10**: Manual testing tasks requiring user to sideload production add-in in Outlook Web, verify HTTPS certificate, and test auto-deploy workflow
