...
- [x] **Task 1: Update EmailContext Component for New Contact Editing** (AC: 1, 2, 3, 5, 8, 9)
  ...
  - [ ] **Progressive Disclosure Logic (AC 9):**
    ...
    - When `matchStatus === 'warning'` (domain-matched): Display editable name + email fields for new contact creation
    - When `matchStatus === 'neutral'` (no-match): Display editable name + email fields for new contact creation
  - [ ] **Add Editable Email Field (AC 3, 5):**
    - Add new prop: `onEmailChange?: (newEmail: string) => void`
    - Render editable email input field when `isEditable === true` (domain-matched or no-match scenarios)
    - Email field pre-filled with `senderEmail` prop value
    - Use same styling as name field: `input` with `border-input`, `rounded-md`, `px-3 py-2`
    - Add email icon (Mail from lucide-react) in input field (similar to Edit2 icon for name)
  - [ ] **Update Props Interface:**
    ...
    export interface EmailContextProps {
      ...
      onEmailChange?: (newEmail: string) => void; // NEW
    }
    ```
  - [ ] **Display Logic Summary:**
    - `matched`: Read-only name (line 67) + read-only email (line 69) → No changes needed (existing)
    - `warning` or `neutral`: Editable name input (lines 56-64) + NEW editable email input → AC 1, 2, 3
...
- [x] **Task 3: Add New Contact Creation Indicator Message** (AC: 7)
  - [ ] Add message component below EmailContext when creating new contact
  - [ ] Display condition: `matchingResult?.type === 'domain-matched' || matchingResult?.type === 'no-match'`
  - [ ] **Message Content (AC 7):**
    - Text: "This will create a new contact: [contactName] ([contactEmail])"
    - Example: "This will create a new contact: John Smith (john.smith@acme.com)"
    - Use dynamic values from `contactName` and `contactEmail` state
  - [ ] **Styling:**
    - Use informational color scheme (blue background): `bg-blue-50 border-blue-200`
    - Text color: `text-blue-800`
    - Icon: Info icon from lucide-react
    - Padding: `px-3 py-2`, rounded corners: `rounded-md`
  - [ ] Insert message in TicketForm between EmailContext and ClientDropdown (around line 86)
  - [ ] Hide message when `matchingResult?.type === 'contact-matched'` (AC 8)
  - [ ] [Source: Epic 5.3 AC 7, UI component patterns from Story 5.1]
...
- [x] **Task 4: Update App.tsx State Management for Contact Editing** (AC: 2, 3, 5)
  ...
  - [ ] Pass props to EmailContext component:
    ...
    <EmailContext
      ...
      onEmailChange={setContactEmail} // NEW callback (AC 5)
    />
    ```
...
- **Focus for Story 5.3:** Add editable email field, validation, and "new contact" indicator message
...
- **Required Changes (Story 5.3):**
  - Add editable email input field when `isEditable === true` (AC 3)
  - Add `onEmailChange` callback prop (AC 5)
  - Email field should match name field styling (lines 56-64)
  - Use Mail icon from lucide-react (similar to Edit2 for name field)
...
### New Contact Creation Indicator Message

**Display Logic (AC 7):**
```tsx
{/* Display indicator message when creating new contact */}
{(matchingResult?.type === 'domain-matched' || matchingResult?.type === 'no-match') && (
...
**Placement:**
- Insert in TicketForm.tsx between EmailContext and ClientDropdown (around line 86)
- Appears ONLY when new contact will be created (`domain-matched` or `no-match` scenarios)
- Hidden when `contact-matched` (AC 8)

**Styling:**
- Informational color scheme (blue): Indicates action without error/warning connotation
- Icon: Info from lucide-react (informational indicator)
- Border + background: `border-blue-200 bg-blue-50` (subtle, non-intrusive)
- Text: `text-blue-800` (readable contrast on blue background)
...
**Modified Files:**
- [outlook-addin/src/components/EmailContext.tsx](../../../outlook-addin/src/components/EmailContext.tsx) - Add editable email field, onEmailChange callback
- [outlook-addin/src/components/TicketForm.tsx](../../../outlook-addin/src/components/TicketForm.tsx) - Add validation for contact name/email, display new contact indicator message
- [outlook-addin/src/App.tsx](../../../outlook-addin/src/App.tsx) - Add contactName/contactEmail state management, pass callbacks to EmailContext
...
- Update EmailContextProps interface with `onEmailChange` callback (AC 5)
- Update TicketFormProps interface with `contactEmail` prop (AC 3)
- Email validation function with explicit return type: `(email: string) => boolean`
- [Source: [coding-standards-and-integration-rules.md#typescript-standards](../architecture/coding-standards-and-integration-rules.md#typescript-standards)]

**React Patterns:**
- Controlled components for editable email field (useState + onChange)
- Conditional rendering for progressive disclosure (AC 9)
- Prop drilling for callbacks (onEmailChange from App.tsx → EmailContext)
- [Source: Story 5.1, 5.2 React patterns]

**Tailwind CSS:**
- Email input field: Same styling as name field (`border-input`, `rounded-md`, `px-3 py-2`)
- Validation errors: `text-red-600 text-sm` (consistent with Story 5.2)
- New contact indicator message: `bg-blue-50 border-blue-200 text-blue-800` (informational)
- [Source: Story 5.1, 5.2 styling patterns]

### Success Criteria

**Story Completion Checklist:**
1. EmailContext displays editable email field for new contact scenarios (AC 1, 3)
2. Contact name pre-filled from sender display name (AC 2)
3. Contact email pre-filled from sender email (AC 3)
4. Client auto-selected from domain match result (AC 4)
5. User can edit contact name and email before submission (AC 5)
6. Validation enforces name required and valid email format (AC 6)
7. "This will create a new contact..." message displays dynamically (AC 7)
8. New contact fields hidden when exact contact match found (AC 8)
9. Progressive disclosure: fields only appear when needed (AC 9)
10. TypeScript compilation passes
11. Linting passes
12. Manual testing confirms all 8 test cases pass

**Definition of Done:**
- EmailContext component updated with editable email field and onEmailChange callback
- TicketForm component validates contact name/email for new contact scenarios
- New contact indicator message displays with dynamic contact name/email
- App.tsx manages contactName/contactEmail state with pre-filling from email metadata
- Progressive disclosure works: read-only for matched contacts, editable for new contacts
...
3. Verify EmailContext displays editable email input field (not read-only text) (AC 1)
4. Verify StatusBadge shows "⚠ New contact at Acme Corp"
5. Verify "This will create a new contact: Jane Doe (jane.doe@acme.com)" message appears (AC 7)
6. Verify client auto-selected from domain match: "Acme Corp" (AC 4)
7. Verify submit button enabled (form valid with auto-selected client)

**Test Case 3: Pre-Filled Contact Fields from Email Metadata (AC 2, 3)**
1. Continue from Test Case 2 (domain match scenario)
2. Verify name field pre-filled with sender display name: "Jane Doe" (AC 2)
3. Verify email field pre-filled with sender email address: "jane.doe@acme.com" (AC 3)
4. Verify both fields are editable (input elements with border, not read-only text)
5. Verify Edit2 icon appears in name field
6. Verify Mail icon appears in email field

**Test Case 4: Edit Contact Name and Email Before Submission (AC 5)**
1. Continue from Test Case 3 (domain match scenario)
2. Click in name field, change "Jane Doe" to "J. Doe"
3. Verify "This will create a new contact..." message updates to "J. Doe (jane.doe@acme.com)" (AC 7)
4. Click in email field, change "jane.doe@acme.com" to "jdoe@acme.com"
5. Verify "This will create a new contact..." message updates to "J. Doe (jdoe@acme.com)" (AC 7)
6. Fill in time: "15m", description: "Test new contact creation"
7. Submit ticket
8. Verify success message appears: "✓ Ticket #[ID] created successfully"
9. Check database: Verify new contact created with name "J. Doe" and email "jdoe@acme.com" linked to client "Acme Corp"
10. Check database: Verify ticket created with new contact's ID
...
**Test Case 6: Validation - Email Required and Format (AC 6)**
1. Domain match scenario (e.g., "jane.doe@acme.com")
2. Clear contact email field completely (delete all text)
3. Attempt to submit ticket
4. Verify validation error appears: "Contact email is required"
5. Type invalid email "notanemail" (no @ symbol)
6. Verify validation error updates to: "Invalid email format"
7. Type invalid email "test@" (incomplete domain)
8. Verify error persists: "Invalid email format"
9. Type valid email "jane.doe@acme.com"
10. Verify error clears immediately
...
3. Verify EmailContext displays editable email input field (AC 1)
...
6. Verify email pre-filled from sender email: "bob@unknown.com" (AC 3)
7. Verify NO "This will create a new contact..." message YET (no client selected)
8. Manually select client from dropdown: "Test Client"
9. Verify "This will create a new contact: Bob Unknown (bob@unknown.com)" message appears (AC 7)
10. Fill in time: "30m", description: "Test no match scenario"
11. Submit ticket
12. Verify success message appears
13. Check database: Verify new contact created with name "Bob Unknown" and email "bob@unknown.com" linked to "Test Client"

**Test Case 8: Progressive Disclosure - Email Change Resets Form (AC 9)**
1. Domain match scenario with edited contact name/email (e.g., changed "Jane Doe" to "J. Doe")
...
3. Verify form resets:
   ...
   - Contact email field populated with new email's sender email (not previous edited email)
...
- Contact name/email pre-filled from sender metadata
- User can edit contact name/email for new contact creation
- Validation errors display for missing/invalid contact data
- "This will create a new contact..." message displays dynamically with edited values
...
- **Task 1 (EmailContext)**: Added editable email field with Mail icon, matching name field styling. Added `onEmailChange` prop to EmailContextProps interface. Progressive disclosure logic already existed from Story 5.1 (`isEditable` flag).
- **Task 2 (Validation)**: Added `contactNameError` and `contactEmailError` state. Implemented `isValidEmail` function with regex `/^[^\s@]+@[^\s@]+\.[^\s@]+$/`. Validation only runs for `domain-matched` or `no-match` scenarios. Updated `isFormValid` to include contact validation check. Displayed inline validation errors in red text.
- **Task 3 (Indicator Message)**: Added blue informational message "This will create a new contact: [name] ([email])" with Info icon. Message appears between validation errors and ClientDropdown, only when `domain-matched` or `no-match`.
- **Task 4 (App.tsx State)**: Added `contactEmail` state alongside existing `contactName` state. Modified useEffect to initialize both from `emailContext`. Added new useEffect to override contact state when `contact-matched`. Passed `onEmailChange={setContactEmail}` to EmailContext and `contactEmail={contactEmail}` to TicketForm.
- **Task 5 (Submission)**: Changed `contactEmail` prop from optional to required in TicketFormProps. Submission logic already correct from previous implementation (includes contactName/contactEmail in payload for new contact scenarios).
- **Task 7 (Compilation/Linting)**: TypeScript compilation passes with no errors (`npx tsc --noEmit`). Linting passes for all modified files (EmailContext.tsx, TicketForm.tsx, App.tsx). Pre-existing linting errors in shadcn/ui components remain unchanged.

### File List

**Modified Files:**
- [outlook-addin/src/components/EmailContext.tsx](../../../outlook-addin/src/components/EmailContext.tsx) - Added editable email field, Mail icon import, onEmailChange prop
- [outlook-addin/src/components/TicketForm.tsx](../../../outlook-addin/src/components/TicketForm.tsx) - Added contact validation state/logic, new contact indicator message, Info icon import, updated contactEmail prop to required
- [outlook-addin/src/App.tsx](../../../outlook-addin/src/App.tsx) - Added contactEmail state management, useEffect for state initialization and override, passed onEmailChange callback
